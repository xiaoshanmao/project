...
#pragma mark - search功能谓词
- (IBAction)searchButtonClicked:(UIButton *)sender {
    
    if (_searchTextField.text && _searchTextField.text.length >0) {
        
        NSPredicate *predicate = [NSPredicate predicateWithFormat:_searchTextField.text];
        NSArray *array = [_concactsArray filteredArrayUsingPredicate:predicate];
        NSMutableString *allString = [NSMutableString string];
        for (concacts *c in array) {
            
            NSString *concactStr = [NSString stringWithFormat:@"%@  |%@  |%@  |%@",c.name,c.gender,c.company,c.occupation];
            [allString appendString:concactStr];
        }
        _resultTextView.text = allString;

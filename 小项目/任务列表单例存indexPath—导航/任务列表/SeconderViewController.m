...
- (void)viewDidLoad {
   ...
   // indexPath_ = [NSIndexPath indexPathForRow:10000 inSection:0];
    ...
    MessageMoel *mlo = [MessageMoel shareInstance];
    
        _taskArray = mlo.taskArray;
    }
...
- (void)creatNav{
    ...
    UIImage *originImage = [UIImage imageNamed:@"保存1"];
    originImage = [originImage imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:originImage style:UIBarButtonItemStylePlain target:self action:@selector(lbuttonClicked:)];
    self.title = @"任务详情";
...
//    [self.navigationController popToRootViewControllerAnimated:YES];
    [self.navigationController popToViewController:self.viewC animated:YES];
//    [self dismissViewControllerAnimated:YES completion:^{
//        
//    }];
    mol = [NSKeyedUnarchiver unarchiveObjectWithFile:_path];
    
    if (mol.taskArray.count==0 || mol.indexPathl.row == 10000) {
        ...
        mol.indexPathl =[NSIndexPath indexPathForRow:10000 inSection:0];
